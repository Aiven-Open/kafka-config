...
public class ExtendedConfigKey extends ConfigDef.ConfigKey {
  ...
  /**
   * Creates an extended config key from a ConfigKey. If the key is already an ExtendedConfigKey it
   * is simply returned.
   *
   * @param key The key to construct the ExtendedConfigKey from.
   ...
   *
   * @return If the deprecation was set, returns {@link DeprecatedInfo#getDescription()} otherwise
  ...
  /**
   * Sets since override. The builder must define {@code version} at a minimum. Any undefined values
   * will be blank.
   *
  ...
  public static class Builder<T extends Builder<?>> extends ConfigKeyBuilder<T> {
    ...
    public static <T extends Builder<?>> Builder<T> unbuild(final ConfigDef.ConfigKey key) {
      ...
      result
          ...
          .group(key.group)
          .orderInGroup(key.orderInGroup)
          .width(key.width)
          .displayName(key.displayName)
          .recommender(key.recommender)
          .internalConfig(key.internalConfig)
