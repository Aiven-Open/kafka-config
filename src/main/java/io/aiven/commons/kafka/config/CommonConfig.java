...
public class CommonConfig extends AbstractConfig {
  ...
  /**
   * Checks the configuration definition for errors. If any errors are found an exception is thrown.
   * Due to the point at which this is called there are no {@link ConfigDef.ConfigKey#validator}
   * errors to worry about.
   *
   * @param definition the ConfigDefinition to validate.
   * @param props The map of parameter name to values to verify with.
   */
  private void doVerification(final CommonConfigDef definition, final Map<String, String> props) {
    ...
    final List<ConfigValue> errorConfigs =
        configValueMap.values().stream()
   ...
   * Constructor.
   *
   * @param definition CommonConfigDef based definition to use.
   * @param originals the original property name to value map.
   */
  ...
  /**
   * Allows implementations to modify the values of the map from the fragments. Default
   * implementation does nothing.
   *
   * @param map the map to make changes in.
   */
  ...
  /**
   * Get the maximum number of tasks that should be run by this connector configuration Max Tasks is
   * set within the Kafka Connect framework and so is retrieved slightly differently in
  ...
  public static class ChangeTrackingMap {
    ...
    /**
     * Sets the override for a key. Passing {@code null} removes any override, any other values sets
     * the override.
     *
    ...
    /**
     * Gets the current value of the key. This is the last override or the current value if no
     * override is present.
     *
