...
public class DeprecationReporter {
   ...
   * @param configDef the definition for the class.
   * @return a list of deprecation notices for deprecated items that have non-default values.
   */
  public static List<String> report(AbstractConfig config, ConfigDef configDef) {
    ...
    return configDef.configKeys().values().stream()
        ...
        .map(ex -> String.format("Option %s", ex.deprecated.formatted(ex.name)))
        .toList();
  }
}
