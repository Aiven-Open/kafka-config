...
public class ConfigDefBeanFactory {
  /**
   * Constructs the ConfigDefBean for the named class that will return ExtendedConfigKeyBeans. If
   * there is a property file in the path specified by the fully qualified className, that has the
   * extension ".versionMap" it will be read and applied as a version map using the {@link
   * SinceInfoMapBuilder}. For example if there is a class "foo.bar.FooBarConfigDef" and a file
   * "foo/bar/FooBarConfigDef.versionMap" found in the properties that file will be read and applied
   * to the configuration definition.
   *
   * @param className the class to construct the bean for.
   * @return the ConfigDefBean for velocity usage.
   */
  public ConfigDefBean<ExtendedConfigKeyBean> open(String className) {
    try {
      final Class<? extends ConfigDef> clazz =
          (Class<? extends ConfigDef>) Class.forName(className);
