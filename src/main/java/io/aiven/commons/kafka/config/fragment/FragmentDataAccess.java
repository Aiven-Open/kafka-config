 ...
 *
 * <p>ConfigFragments require access to the parameter and string values from the configuration, but
 * have different types of data avialable to extract that data from. This interface defines the
 * methods that the ConfigFragment may use to retrieve data as well as providing implementations
 * against both the {@link AbstractConfig} and a map of String to {@link ConfigValue}.
 */
public interface FragmentDataAccess {
  ...
  static FragmentDataAccess from(final Map<String, ConfigValue> configValues) {
    return new FragmentDataAccess() {
      ...
      public Map<String, ?> values() {
        ...
        configValues
            .values()
            .forEach(configValue -> values.put(configValue.name(), configValue.value()));
        return values;
      }
   ...
   * @return {@code true} if the key was set, {@code false} if the key was not set or does not exist
   *     in the config.
   */
