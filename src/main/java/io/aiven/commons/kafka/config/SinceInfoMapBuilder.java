...
/**
 * Class to build and maintain a list of Override ranges to final version. Items are retained in the
 * order in which they were added.
 */
public class SinceInfoMapBuilder {
  /**
   * The comment character for comment lines in input files. The comment must be the first character
   * in the line.
   */
   ...
   *
   * @param range the OverrideRange.
   * @param data the final version data.
   */
   ...
   *
   * @param groupId the group ID for the OverrideRange.
   * @param artifactId the artifact ID for the OverrideRange.
   * @param version the version for the OverrideRange.
   * @param data the final version data.
   */
  public void put(String groupId, String artifactId, String version, SinceInfo.Data data) {
    try {
      put(
          new SinceInfo.OverrideRange(
              SinceInfo.builder().groupId(groupId).artifactId(artifactId).version(version)),
          data);
   ...
   *
   * @param pattern a pattern in the form groupId:artifactId:versionRange
   * @param data the final version data.
   */
   ...
   *
   * @param range the override range.
   * @param finalVersion the final version.
   */
   ...
   * @param artifactId the artifact ID for the OverrideRange.
   * @param version the version for the OverrideRange.
   * @param finalVersion the final version data.
   */
   ...
   *
   * @param printStream the stream to serialize to.
   */
  ...
  /**
   * Read the map from an input stream. The stream must comprise lines of text in the form
   * groupId:artifactId:versionRange:finalVersion Parser ignores empty lines and lines starting with
   * '#'
   *
   * @param inputStream the input stream to read from.
   * @throws IOException on input error or format error.
   */
        ...
        if (!(line.isEmpty() || line.startsWith(COMMENT_CHAR))) {
          ...
          if (parts.length < 4 || parts.length > 6) {
            throw new IOException(
                String.format(
                    "Invalid structure: %s nust have 4 to 6 parts separated by ':'", line));
          }
  ...
  /**
   * Applies the mapping to s the final versions for the specified Since values of the
   * ExtendedConfigKeys in the ConfigDef.
   *
   * @param configDef the configDef to adjust.
   */
