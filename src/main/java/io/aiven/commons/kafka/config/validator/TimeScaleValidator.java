...
public class TimeScaleValidator implements ConfigDef.Validator {
  ...
  TimeScaleValidator(final Number minMilliseconds, final Number maxMilliseconds) {
    ...
    this.minScale =
        minMilliseconds == null ? TimeScale.MILLISECONDS : TimeScale.scaleOf(min.longValue());
    this.max = maxMilliseconds;
    this.maxScale =
        maxMilliseconds == null ? TimeScale.MILLISECONDS : TimeScale.scaleOf(max.longValue());
  }
   ...
   * @return A times scale validator that requires a value of at least {@code min}.
   */
  ...
  public void ensureValid(final String name, final Object value) {
    ...
    if (min != null && number.longValue() < min.longValue()) {
      throw new ConfigException(
          name, value, "Value must be at least " + minScale.displayValue(min.longValue()));
    }
    if (max != null && number.doubleValue() > max.doubleValue()) {
      throw new ConfigException(
          name, value, "Value must be no more than " + maxScale.displayValue(max.longValue()));
    }
  }
  ...
  public String toString() {
    if (min == null) {
      ...
      return "["
          + minScale.format(min.longValue())
          + ",...,"
          + maxScale.format(max.longValue())
          + "]";
    }
  }
}
