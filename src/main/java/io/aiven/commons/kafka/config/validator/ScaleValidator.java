...
public class ScaleValidator implements ConfigDef.Validator {
   ...
   * @param possibleScales the potential scales for display.
   */
  ScaleValidator(final Number minBytes, final Number maxBytes, final List<Scale> possibleScales) {
    this.minBytes = minBytes;
    this.minScale =
        minBytes == null ? Scale.B : Scale.scaleOf(minBytes.longValue(), possibleScales);
    this.maxBytes = maxBytes;
    this.maxScale =
        maxBytes == null ? Scale.B : Scale.scaleOf(maxBytes.longValue(), possibleScales);
  }

  /**
   * A scale range that checks only the lower bound
   *
   * @param min The minimum acceptable number of bytes
   * @param possibleScales The list of potential scale values.
   ...
   *
   * @param min the minimum acceptable number of bytes.
   * @param max the maximum acceptable number of bytes.
   * @param possibleScales The list of potential scale values.
   * @return A scale validator that requires a value between the {@code min} and {@code max} numbers
   *     inclusive.
   */
  public static ScaleValidator between(
      final Number min, final Number max, final List<Scale> possibleScales) {
    return new ScaleValidator(min, max, possibleScales);
  }

  @Override
  public void ensureValid(final String name, final Object value) {
    if (value == null) throw new ConfigException(name, null, "Value must be non-null");
    final Number number = (Number) value;
    if (minBytes != null && number.longValue() < minBytes.longValue())
      throw new ConfigException(
          name, value, "Value must be at least " + minScale.displayValue(minBytes.longValue()));
    if (maxBytes != null && number.doubleValue() > maxBytes.doubleValue())
      throw new ConfigException(
          name, value, "Value must be no more than " + maxScale.displayValue(maxBytes.longValue()));
  }

  @Override
  public String toString() {
    if (minBytes == null) return "[...," + maxScale.format(maxBytes.longValue()) + "]";
    else if (maxBytes == null) return "[" + minScale.format(minBytes.longValue()) + ",...]";
    else
      return "["
          + minScale.format(minBytes.longValue())
          + ",...,"
          + maxScale.format(maxBytes.longValue())
          + "]";
  }
}
