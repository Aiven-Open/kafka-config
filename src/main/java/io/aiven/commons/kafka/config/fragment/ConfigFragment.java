 ...
 *
 * <p>All implementing classes should be final. All configuration keys should be specified as {@code
 * static final String} within the fragment. They should be package private so that test code can
 * use them. All implementing classes must implement a {@code public static ConfigDef update(final
 * ConfigDef configDef)} method, though more arguments are allowed. The method must add the
 * definitions for the configuration options the fragment supports to the configDef parameter and
 * return it. Access to the configuration within the fragment is via the {@link #dataAccess}
 * property. All access to configuration values should be through methods. For example if the string
 * value of the "foo" config options should be returned then the method { @code public String
 * getFoo() } should be created. All complex access to configuration values should be encapsulated
 * within methods. For example if the Baz object requires the "foo" and "bar" values then something
 * like {@code public Baz getBaz() { return new Baz(getFoo(), cfg.getString("bar")); }} should be
 * created. Any fragment depends on another fragment may create it inline and use it. The {@link
 * #validate(Map)} method may call the {@code validate} methods on the dependant fragments during
 * validation.
 */
public class ConfigFragment implements FragmentDataAccess {
   ...
   */
  public static void logDeprecated(
      final Logger logger, final String old, final String replacement) {
    logDeprecated(logger, old, "Use property %s instead", replacement);
  }

  /**
   * Logs a deprecated message for a deprecated configuration key. This method should only be used
   * when the key does not have a replacement and the documentation describes what to do.
   *
   * @param logger the logger to log to.
   * @param old the deprecated configuration key.
   * @param formatStr the string to {@link String#format} the args.
   * @param args the arguments to format with the {@code formatStr}
   */
  public static void logDeprecated(
      final Logger logger, final String old, final String formatStr, final Object... args) {
  ...
  /**
   * Validate that the data in the configuration matches any restrictions other than those from the
   * validator. Default implementation does nothing.
   *
   * @param configMap The map of ConfigValues that have issues
   */
  public void validate(final Map<String, ConfigValue> configMap) {}

  /**
   * Validate that the data in the configuration matches any restrictions. Default implementation
   * does nothing.
   */
  @Deprecated
