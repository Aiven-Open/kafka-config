...
/**
 * Contains the information about when this option was added. Since options can be added at various
 * stages and by different libraries and because the "since" field is to assist users in determining
 * at what version an option became available in the final connector the SinceInfo tracks the module
 * and version where the option was added but provides a mechanism to override the display in the
 * final configuration.
 */
public final class SinceInfo {
   ...
   */
  public record Data(String groupId, String artifactId, String version)
    ...
    public int compareTo(Data other) {
      int result =
          StringUtils.defaultIfEmpty(groupId, "")
              .compareTo(StringUtils.defaultIfEmpty(other.groupId, ""));
      if (result == 0) {
        result =
            StringUtils.defaultIfEmpty(artifactId, "")
  ...
  /**
   * Constructs the SinceInfo from the builder. The builder must define {@code groupId}, {@code
  ...
  /**
   * Sets the override for this SinceInfo. The builder must define {@code version}. The {@code
   * groupId} and {@code artifactId} are optional and will be displayed if present.
   *
  ...
  /**
   * Sets the override from the Builder associated with the matching OverrideRange in the
   * overrideMap. If multiple ranges match the last one in the map will be applied.
   *
  ...
  public void setOverride(Map<OverrideRange, Data> overrideMap) {
    overrideMap.entrySet().stream()
        .filter(e -> e.getKey().matches(this))
  ...
  public static final class OverrideRange implements Comparable<OverrideRange> {
    ...
    /**
     * Creates an OverrideRange from a SinceInfo.Builder. The version specified may be a range as
     * defined in the Maven method {@link VersionRange#createFromVersionSpec(String)}.
     *
