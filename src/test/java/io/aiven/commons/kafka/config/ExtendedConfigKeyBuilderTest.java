...
public class ExtendedConfigKeyBuilderTest {
  ...
  void testDeprecation() {
    ExtendedConfigKey key =
        ExtendedConfigKey.builder("deprecation")
  ...
  void testSince() {
    SinceInfo sinceInfo =
        SinceInfo.builder().artifactId("artifactId").groupId("groupId").version("version").build();
  ...
  void testOverrideSince() {
    SinceInfo.Builder sinceBuilder =
        SinceInfo.builder().artifactId("artifactId").groupId("groupId").version("1.0");
    SinceInfo.Builder deprecatedSinceBuilder =
        SinceInfo.builder().artifactId("artifactId").groupId("groupId").version("1.5");
    SinceInfo.Builder overrideBuilder = SinceInfo.builder().version("myVersion");
    // since only
    ExtendedConfigKey key =
        ExtendedConfigKey.builder("overrideSince").since(sinceBuilder.build()).build();
    ...
    key =
        ExtendedConfigKey.builder("overrideSince")
            .since(sinceBuilder.build())
            .deprecatedInfo(DeprecatedInfo.builder().since(deprecatedSinceBuilder))
            .build();
    assertThat(key.getSince()).isEqualTo("groupId:artifactId:1.0");
