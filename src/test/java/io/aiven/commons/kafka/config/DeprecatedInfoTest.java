...
public class DeprecatedInfoTest {
  ...
  void testSince() {
    ...
    assertThat(underTest.formatted("emptyTest"))
        .isEqualTo("emptyTest is deprecated since myVersion");
  }
  ...
  void testSinceOverrideMap() throws InvalidVersionSpecificationException {
    SinceInfo.Builder builder =
        SinceInfo.builder().groupId("io.aiven.commons").artifactId("kafka-config").version("1.0");
    SinceInfo since = builder.build();
    DeprecatedInfo underTest = DeprecatedInfo.builder().since(since).build();
    assertThat(underTest.formatted("emptyTest"))
        .isEqualTo("emptyTest is deprecated since io.aiven.commons:kafka-config:1.0");
    Map<SinceInfo.OverrideRange, SinceInfo.Data> overrideMap =
        Map.of(
  ...
  void testForRemovalSince() {
    ...
    assertThat(underTest.formatted("emptyTest"))
        .isEqualTo(
            "emptyTest is deprecated for removal since io.aiven.commons:kafka-config:LATEST");
  }

  @Test
  void testForRemovalDescription() {
    DeprecatedInfo underTest =
        DeprecatedInfo.builder().description("why not").forRemoval(true).build();
    assertThat(underTest.formatted("emptyTest"))
        .isEqualTo("emptyTest is deprecated for removal: why not");
  }
  ...
  void testSinceDescriptionForRemoval() {
    SinceInfo since =
        SinceInfo.builder()
            ...
            .build();
    DeprecatedInfo underTest =
        DeprecatedInfo.builder().description("why not").since(since).forRemoval(true).build();
    assertThat(underTest.formatted("emptyTest"))
        .isEqualTo(
            "emptyTest is deprecated for removal since io.aiven.commons:kafka-config:LATEST: why not");
  }
}
